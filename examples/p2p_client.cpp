...
//#define STREAM_SERVER_TEST
//#define KCP_BASE_TEST
#define DOWNLAND_TEST
#ifdef KCP_BASE_TEST
...
int main(int argc,char *argv[])
{
    ...
    if(mode=="server")
    {
        ...
        client.stream_client_init("micagent",10,[](std::string ip,int port,std::string external_ip,int external_port){std::cout<<ip<<":"<<port<<std::endl;});
    }
...
int main(int argc,char *argv[])
{
    ...
    sensor_net::KCP_Manager::GetInstance().Config(event_loop.get());
    //添加KCP 状态处理任务
    sensor_net::KCP_Manager::GetInstance().StartUpdateLoop(10);
    p2p_punch_client client(argv[1],argv[2],event_loop);
    std::thread t(std::bind(&xop::EventLoop::loop,event_loop));
    t.detach();
    std::string mode=argv[3];
    ...
    {//可人为区分
    downland_test=true;
        client.setTunnelEventCallback(kcp_test::downland_eventcallback);
    }
    else {
    downland_test=false;
        client.setTunnelEventCallback(kcp_test::downland_eventcallback);
    ...
    if(mode!="server")
    {
        ...
        while(1)
        {
            std::cout<<"input device_id:"<<std::endl;
            std::cin>>device_id;
            std::cout<<"input source_name:"<<std::endl;
            std::cin>>source_name;
            std::cout<<"input mode 0 for kcp 1 for kcp_active 2 for kcp_tunnel:"<<std::endl;
            std::cin>>mode;
            if(mode<0||mode>2)
            {
                std::cout<<"false mode input! please input again!"<<std::endl;
                continue;
            }
            else {
                int port=0;
                if(mode==1)port=kcp_test::downland_get_active_port(event_loop);
                client.try_set_up_data_tunnel(device_id,source_name,static_cast<p2p_punch_client::TUNNEL_TYPE>(mode),port);
            }
        }
    }
    while(getchar()!='8')continue;

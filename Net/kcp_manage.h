...
namespace sensor_net{
    typedef std::function<void(std::shared_ptr<char> buf,int len,struct IKCPCB *kcp,void *user)> RecvDataCallback;
    ...
    public:
        KCP_Interface(int fd,unsigned int conv_id,xop::EventLoop *event_loop,ClearCallback clearCB,RecvDataCallback recvCB,void *data,int window_size=32);
        KCP_Interface(std::shared_ptr<xop::Channel>channel,unsigned int conv_id,xop::EventLoop *event_loop,ClearCallback clearCB,RecvDataCallback recvCB,void *data,int window_size=32);
        //发送用户数据
    ...
    private:
        ...
        struct IKCPCB *m_kcp;
        std::atomic<bool> m_have_cleared;
    ...
    public:
        ...
        std::shared_ptr<KCP_Interface> AddConnection(int fd,std::string ip,int port,unsigned int conv_id,RecvDataCallback recvCB,void *data,int window_size=32);
        std::shared_ptr<KCP_Interface> AddConnection(std::shared_ptr<xop::Channel> channel,unsigned int conv_id,RecvDataCallback recvCB,void *data,int window_size=32);
